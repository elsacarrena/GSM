...
try {
    ...
    public function  EnregistrernewEmploye(Request $request)
    {
            $request->validate([
                'name' => ['required', 'string', 'max:255'],
                'email' => ['required', 'string', 'email', 'max:255', 'unique:users'],
        ...
        $id=$findUserByEmail->id;
        $userName=$findUserByEmail->name;
        Mail::to($request->email)->send(new ComfirmemployeMailActiver($id,$userName));
        // Notification de l'utilisateur inscrit
        // event(new Registered($employe));
        // $employe->notify(new RegisteredUser());

        return redirect('/form_employe')->with('success', 'Votre compte a été bien créé. Un mail de creation de compte vous a été envoyé dans votre adresse mail.');
    }
